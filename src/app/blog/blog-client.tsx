...
function BlogFilterContent() {
  ...
  return (
    <div className="py-8 sm:py-12 bg-slate-50 min-h-screen">
      <Container>
        {/* Breadcrumbs */}
        <Breadcrumbs items={[{ label: "Solar Guide & Knowledge Base" }]} />

        {/* Header */}
        <div className="mt-4 mb-10">
          <SectionHeader
            badge="Engineering Knowledge Base"
            title="Solar Energy Guides & Technical Insights for Bangladesh"
            subtitle="Authoritative articles on net metering policy, system sizing formulas, battery chemistry comparisons, and industrial solar economics."
          />
        </div>
        ...
        <div className="mb-12 space-y-4">
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            ...
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                type="text"
                aria-label="Search solar guides and articles"
                placeholder="Search solar guides, net metering, sizing..."
                value={searchQuery}
            ...
            <div className="text-xs text-slate-500 font-medium">
              Showing <span className="font-bold text-slate-900">{filteredArticles.length}</span>{" "}
              expert articles
            </div>
          </div>

          {/* Category Chips */}
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none" role="group" aria-label="Filter articles by category">
            {categories.map((cat) => {
